...
namespace ENTITY
{
  ...
  /// </summary>
  public sealed class EP_FolderPath : E_Attribute
  {
  ...
  /// </summary>
  [CustomPropertyDrawer(typeof(EP_FolderPath))]
  public sealed class EPD_FolderPath : E_PropertyDrawer
  {
    ...
    private string CompletePath(SerializedProperty property, EP_FolderPath attribute)
    {
      string parent_folder = attribute.ParentFolder; // Get the parent folder.
      if (parent_folder.First().ToString() == VariableCall)
        parent_folder = CheckDynamicField(property.serializedObject.targetObject, parent_folder, "");
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
      ...
      {
        EP_FolderPath attribute = this.attribute as EP_FolderPath; // Get the attribute being targeted.
        ...
        GUIContent folderGUI = new GUIContent(folderIcon, "Select Folder");
        string path = property.stringValue;
        // Check if the button was clicked.
        if (GUI.Button(position, folderGUI))
        {
          path = EditorUtility.OpenFolderPanel("Select a Folder", CompletePath(property, attribute), ""); // Get the path from the folder panel.
